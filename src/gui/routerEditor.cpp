...
ImU32 RouterEditor::linkColor(const RouterOption& opt) {
    if (opt.mutator.setActionType.has_value())
        return IM_COL32(220,  80,  80, 255); // red   – mutating
    if (!opt.condition.conditions.empty())
        return IM_COL32(230, 160,  40, 255); // orange – conditional
    return     IM_COL32( 80, 200, 120, 255); // green  – always
}

// Lifecycle
RouterEditor::RouterEditor(SDL_Window* window, std::function<void(const std::string&)> onLoad, std::function<void(const std::string&)> onSave)
    : sdlWindow(window), onLoad(std::move(onLoad)) , onSave(std::move(onSave)) {
...
void SDLCALL RouterEditor::fileDialogLoadCB(void* userdata,
...
void SDLCALL RouterEditor::fileDialogSaveCB(void* userdata,
                                             const char* const* filelist,
                                             int /*filter*/)
{
    auto* p = static_cast<PendingPath*>(userdata);
...
void RouterEditor::flushPendingPaths() {
    ...
    }
    {
        ...
        if (pendingSave.ready) {
            if (onSave) onSave(pendingSave.path);
...
// Top-level render
void RouterEditor::render(RouterData& routerData) {
    flushPendingPaths();

    renderToolbar(routerData);
    ImGui::Separator();

    rebuildLinkMeta(routerData);
    ...
    renderGraph(routerData);
    ImGui::EndChild();

    ImGui::SameLine();
    ImGui::BeginChild("##inspector", { kInspectorWidth, 0 }, ImGuiChildFlags_Borders);
    renderInspector(routerData);
    ImGui::EndChild();
}

// Toolbar
void RouterEditor::renderToolbar(RouterData& routerData) {
    // ---- File I/O ----
    ...
    if (ImGui::Button("Load Config")) {
        SDL_ShowOpenFileDialog(fileDialogLoadCB,
                               &pendingLoad,
                               sdlWindow,
                               kJsonFilter, 2,
                               nullptr,
    ...
    if (ImGui::Button("Save Config")) {
        SDL_ShowSaveFileDialog(fileDialogSaveCB,
                               &pendingSave,
                               sdlWindow,
                               kJsonFilter, 2,
                               nullptr);
    ...
    //     RouterOption opt;
    //     opt.output = { SRAM_CACHE, 0 };
    //     routerData[SRAM_CACHE][viewedAction].push_back(opt);
    //     int newIdx = (int)routerData[SRAM_CACHE][viewedAction].size() - 1;
    //     selectedLink = linkId(SRAM_CACHE, viewedAction, newIdx);
    // }
    ImGui::SameLine();
...
// Link metadata
void RouterEditor::rebuildLinkMeta(const RouterData& routerData) {
    linkMetaMap.clear();
    for (const auto& [srcBlock, actionMap] : routerData) {
...
// Node graph
void RouterEditor::renderGraph(RouterData& routerData) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (!positionsSet)
            nodeEditor::SetNodePosition(nodeId(bt), nodeLayout(bt));
        drawNode(bt);
    }
    positionsSet = true;

    for (const auto& [lid, meta] : linkMetaMap) {
        const auto& vec = routerData.at(meta.srcBlock).at(meta.srcAction);
        const RouterOption& opt = vec[meta.optionIndex];
        nodeEditor::Link(lid,
                 outputPinId(meta.srcBlock, 0),
                 inputPinId(opt.output.first, opt.output.second),
                 ImGui::ColorConvertU32ToFloat4(linkColor(opt)), 2.5f);
    }

    if (selectedLink.has_value())
        nodeEditor::SelectLink(*selectedLink, false);

    handleCreation(routerData);
    handleDeletion(routerData);
...
void RouterEditor::handleCreation(RouterData& routerData) {
    if (!nodeEditor::BeginCreate(ImColor(255, 255, 255), 2.0f)) { nodeEditor::EndCreate(); return; }
    ...
    if (nodeEditor::QueryNewLink(&startPin, &endPin) && startPin && endPin) {
        ...
        if (!startIsOutput || !endIsInput) {
            ...
            if (nodeEditor::AcceptNewItem(ImColor(80, 200, 120), 2.0f)) {
                ...
                routerData[srcBlock][viewedAction].push_back(opt);
                rebuildLinkMeta(routerData);
                int newIdx = (int)routerData[srcBlock][viewedAction].size() - 1;
...
void RouterEditor::handleDeletion(RouterData& routerData) {
    if (!nodeEditor::BeginDelete()) { nodeEditor::EndDelete(); return; }
    ...
    while (nodeEditor::QueryDeletedLink(&deletedId)) {
        ...
        auto& vec = routerData[src][action];
        if (idx >= 0 && idx < (int)vec.size())
            vec.erase(vec.begin() + idx);
        if (selectedLink == deletedId)
            selectedLink = std::nullopt;
        rebuildLinkMeta(routerData);
    }
...
// Inspector
void RouterEditor::renderInspector(RouterData& routerData) {
    if (!selectedLink.has_value()) { ImGui::TextDisabled("Click a link to inspect it."); return; }

    const int lid = (int)selectedLink->Get();
    auto it = linkMetaMap.find(lid);
    if (it == linkMetaMap.end()) { ImGui::TextDisabled("(stale selection)"); return; }

    auto& [srcBlock, action, idx] = it->second;
    auto& vec = routerData[srcBlock][action];
    ...
    static_assert(sizeof(kBlockNames) / sizeof(kBlockNames[0]) == 6,
                  "kBlockNames must match BlockType count");
    ...
    if (ImGui::InputInt("Port##dst", &port))
        opt.output.second = std::clamp(port, 0, 1);
...
void RouterEditor::renderConditionEditor(Condition& cond) {
    ...
    if (ImGui::SmallButton("+ Add"))
        cond.conditions.emplace_back("arg", EQUAL, 0);

    if (cond.conditions.empty()) { ImGui::TextDisabled("  (none — always fires)"); return; }
    ...
    for (int i = 0; i < (int)cond.conditions.size(); i++) {
        ...
        if (ImGui::Combo("##c", &cmpI, kCompareOps, 6))
            compare = static_cast<Compare>(cmpI);
    ...
    if (eraseAt >= 0)
        cond.conditions.erase(cond.conditions.begin() + eraseAt);
}
