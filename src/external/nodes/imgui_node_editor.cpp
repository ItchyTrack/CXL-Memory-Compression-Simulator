...
//------------------------------------------------------------------------------
namespace nodeEditor = ax::NodeEditor::Detail;
...
void nodeEditor::Log(const char* fmt, ...)
{
# if defined(_DEBUG) && defined(_WIN32)
    va_list args;
    va_start(args, fmt);
...
//------------------------------------------------------------------------------
static bool IsGroup(const nodeEditor::Node* node)
{
    if (node && node->m_Type == nodeEditor::NodeType::Group)
        return true;
    else
        return false;
}
...
//
//------------------------------------------------------------------------------
void nodeEditor::Pin::Draw(ImDrawList* drawList, DrawFlags flags)
{
    if (flags & Hovered)
    {
...
ImVec2 nodeEditor::Pin::GetClosestPoint(const ImVec2& p) const
{
    auto pivot  = m_Pivot;
...
ImLine nodeEditor::Pin::GetClosestLine(const Pin* pin) const
{
    auto pivotA  =      m_Pivot;
...
//
//------------------------------------------------------------------------------
bool nodeEditor::Node::AcceptDrag()
{
    m_DragStart = m_Bounds.Min;
    return true;
}

void nodeEditor::Node::UpdateDrag(const ImVec2& offset)
{
...
bool nodeEditor::Node::EndDrag()
{
    return m_Bounds.Min != m_DragStart;
}

void nodeEditor::Node::Draw(ImDrawList* drawList, DrawFlags flags)
{
...
void nodeEditor::Node::DrawBorder(ImDrawList* drawList, ImU32 color, float thickness, float offset)
{
    if (thickness > 0.0f)
    {
...
void nodeEditor::Node::GetGroupedNodes(std::vector<Node*>& result, bool append)
{
    if (!append)
        result.resize(0);
...
ImRect nodeEditor::Node::GetRegionBounds(NodeRegion region) const
{
...
nodeEditor::NodeRegion nodeEditor::Node::GetRegion(const ImVec2& point) const
{
    if (m_Type == NodeType::Node)
    {
...
//
//------------------------------------------------------------------------------
void nodeEditor::Link::Draw(ImDrawList* drawList, DrawFlags flags)
{
    if (flags == None)
    {
...
void nodeEditor::Link::Draw(ImDrawList* drawList, ImU32 color, float extraThickness) const
{
    if (!m_IsLive)
        return;
...
void nodeEditor::Link::UpdateEndpoints()
{
...
ImCubicBezierPoints nodeEditor::Link::GetCurve() const
{
...
bool nodeEditor::Link::TestHit(const ImVec2& point, float extraThickness) const
{
    if (!m_IsLive)
        return false;
...
bool nodeEditor::Link::TestHit(const ImRect& rect, bool allowIntersect) const
{
    if (!m_IsLive)
        return false;
...
ImRect nodeEditor::Link::GetBounds() const
{
    if (m_IsLive)
    {
        const auto curve = GetCurve();
...
//
//------------------------------------------------------------------------------
nodeEditor::EditorContext::EditorContext(const ax::NodeEditor::Config* config)
    : m_Config(config)
    , m_EditorActiveId(0)
    , m_IsFirstFrame(true)
    , m_IsFocused(false)
...
nodeEditor::EditorContext::~EditorContext()
{
    if (m_IsInitialized)
        SaveSettings();
...
void nodeEditor::EditorContext::Begin(const char* id, const ImVec2& size)
{
...
void nodeEditor::EditorContext::End()
{
    //auto& io          = ImGui::GetIO();
...
bool nodeEditor::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness)
{
    //auto& editorStyle = GetStyle();
...
void nodeEditor::EditorContext::SetNodePosition(NodeId nodeId, const ImVec2& position)
{
...
void nodeEditor::EditorContext::SetGroupSize(NodeId nodeId, const ImVec2& size)
{
    auto node = FindNode(nodeId);
    if (!node)
    {
...
ImVec2 nodeEditor::EditorContext::GetNodePosition(NodeId nodeId)
{
    auto node = FindNode(nodeId);
    if (!node)
...
ImVec2 nodeEditor::EditorContext::GetNodeSize(NodeId nodeId)
{
    auto node = FindNode(nodeId);
    if (!node)
        return ImVec2(0, 0);
...
void nodeEditor::EditorContext::SetNodeZPosition(NodeId nodeId, float z)
{
    auto node = FindNode(nodeId);
    if (!node)
    {
...
float nodeEditor::EditorContext::GetNodeZPosition(NodeId nodeId)
{
    auto node = FindNode(nodeId);
    if (!node)
        return 0.0f;
...
void nodeEditor::EditorContext::MarkNodeToRestoreState(Node* node)
{
    node->m_RestoreState = true;
}

void nodeEditor::EditorContext::UpdateNodeState(Node* node)
{
...
void nodeEditor::EditorContext::RemoveSettings(Object* object)
{
...
void nodeEditor::EditorContext::ClearSelection()
{
...
void nodeEditor::EditorContext::SelectObject(Object* object)
{
    m_SelectedObjects.push_back(object);
    object->m_IsSelected = true;
}

void nodeEditor::EditorContext::DeselectObject(Object* object)
{
...
void nodeEditor::EditorContext::SetSelectedObject(Object* object)
{
    ClearSelection();
    SelectObject(object);
}

void nodeEditor::EditorContext::ToggleObjectSelection(Object* object)
{
...
bool nodeEditor::EditorContext::IsSelected(Object* object)
{
...
const nodeEditor::vector<nodeEditor::Object*>& nodeEditor::EditorContext::GetSelectedObjects()
{
    return m_SelectedObjects;
}

bool nodeEditor::EditorContext::IsAnyNodeSelected()
{
    for (auto object : m_SelectedObjects)
        if (object->AsNode())
            return true;
...
bool nodeEditor::EditorContext::IsAnyLinkSelected()
{
    for (auto object : m_SelectedObjects)
        if (object->AsLink())
            return true;
...
bool nodeEditor::EditorContext::HasSelectionChanged()
{
    return m_LastSelectedObjects != m_SelectedObjects;
}

nodeEditor::Node* nodeEditor::EditorContext::FindNodeAt(const ImVec2& p)
{
    for (auto node : m_Nodes)
        if (node->TestHit(p))
            return node;
...
void nodeEditor::EditorContext::FindNodesInRect(const ImRect& r, vector<Node*>& result, bool append, bool includeIntersecting)
{
    if (!append)
        result.resize(0);
...
void nodeEditor::EditorContext::FindLinksInRect(const ImRect& r, vector<Link*>& result, bool append)
{
    if (!append)
        result.resize(0);
...
bool nodeEditor::EditorContext::HasAnyLinks(NodeId nodeId) const
{
    for (auto link : m_Links)
    {
        if (!link->m_IsLive)
...
bool nodeEditor::EditorContext::HasAnyLinks(PinId pinId) const
{
    for (auto link : m_Links)
    {
        if (!link->m_IsLive)
...
int nodeEditor::EditorContext::BreakLinks(NodeId nodeId)
{
    int result = 0;
    for (auto link : m_Links)
    {
...
int nodeEditor::EditorContext::BreakLinks(PinId pinId)
{
    int result = 0;
    for (auto link : m_Links)
    {
...
void nodeEditor::EditorContext::FindLinksForNode(NodeId nodeId, vector<Link*>& result, bool add)
{
    if (!add)
        result.clear();
...
bool nodeEditor::EditorContext::PinHadAnyLinks(PinId pinId)
{
...
void nodeEditor::EditorContext::NotifyLinkDeleted(Link* link)
{
    if (m_LastActiveLink == link)
        m_LastActiveLink = nullptr;
}

void nodeEditor::EditorContext::Suspend(SuspendFlags flags)
{
...
void nodeEditor::EditorContext::Resume(SuspendFlags flags)
{
...
bool nodeEditor::EditorContext::IsSuspended()
{
    return m_Canvas.IsSuspended();
}

bool nodeEditor::EditorContext::IsFocused()
{
    return m_IsFocused;
}

bool nodeEditor::EditorContext::IsHovered() const
{
    return m_IsHovered;
}

bool nodeEditor::EditorContext::IsHoveredWithoutOverlapp() const
{
    return m_IsHoveredWithoutOverlapp;
}

bool nodeEditor::EditorContext::CanAcceptUserInput() const
{
    return m_IsFocused && m_IsHovered;
}

int nodeEditor::EditorContext::CountLiveNodes() const
{
    return (int)std::count_if(m_Nodes.begin(),  m_Nodes.end(),  [](const Node* node)  { return node->m_IsLive; });
}

int nodeEditor::EditorContext::CountLivePins() const
{
    return (int)std::count_if(m_Pins.begin(),   m_Pins.end(),   [](const Pin*  pin)   { return pin->m_IsLive; });
}

int nodeEditor::EditorContext::CountLiveLinks() const
{
    return (int)std::count_if(m_Links.begin(),  m_Links.end(),  [](const Link* link)  { return link->m_IsLive; });
}

nodeEditor::Pin* nodeEditor::EditorContext::CreatePin(PinId id, PinKind kind)
{
...
nodeEditor::Node* nodeEditor::EditorContext::CreateNode(NodeId id)
{
    IM_ASSERT(nullptr == FindObject(id));
...
nodeEditor::Link* nodeEditor::EditorContext::CreateLink(LinkId id)
{
    IM_ASSERT(nullptr == FindObject(id));
...
nodeEditor::Node* nodeEditor::EditorContext::FindNode(NodeId id)
{
    return FindItemInLinear(m_Nodes, id);
}

nodeEditor::Pin* nodeEditor::EditorContext::FindPin(PinId id)
{
    return FindItemIn(m_Pins, id);
}

nodeEditor::Link* nodeEditor::EditorContext::FindLink(LinkId id)
{
    return FindItemIn(m_Links, id);
}

nodeEditor::Object* nodeEditor::EditorContext::FindObject(ObjectId id)
{
    if (id.IsNodeId())
...
nodeEditor::Node* nodeEditor::EditorContext::GetNode(NodeId id)
{
...
nodeEditor::Pin* nodeEditor::EditorContext::GetPin(PinId id, PinKind kind)
{
...
nodeEditor::Link* nodeEditor::EditorContext::GetLink(LinkId id)
{
...
void nodeEditor::EditorContext::LoadSettings()
{
...
void nodeEditor::EditorContext::SaveSettings()
{
    m_Config.BeginSave();
...
void nodeEditor::EditorContext::MakeDirty(SaveReasonFlags reason)
{
    m_Settings.MakeDirty(reason);
}

void nodeEditor::EditorContext::MakeDirty(SaveReasonFlags reason, Node* node)
{
    m_Settings.MakeDirty(reason, node);
}

nodeEditor::Link* nodeEditor::EditorContext::FindLinkAt(const ImVec2& p)
{
    for (auto& link : m_Links)
...
ImU32 nodeEditor::EditorContext::GetColor(StyleColor colorIndex) const
{
    return ImColor(m_Style.Colors[colorIndex]);
}

ImU32 nodeEditor::EditorContext::GetColor(StyleColor colorIndex, float alpha) const
{
    auto color = m_Style.Colors[colorIndex];
    return ImColor(color.x, color.y, color.z, color.w * alpha);
}

int nodeEditor::EditorContext::GetNodeIds(NodeId* nodes, int size) const
{
    if (size <= 0)
        return 0;
...
void nodeEditor::EditorContext::RegisterAnimation(Animation* animation)
{
    m_LiveAnimations.push_back(animation);
}

void nodeEditor::EditorContext::UnregisterAnimation(Animation* animation)
{
...
void nodeEditor::EditorContext::UpdateAnimations()
{
...
void nodeEditor::EditorContext::Flow(Link* link, FlowDirection direction)
{
    m_FlowAnimationController.Flow(link, direction);
}

void nodeEditor::EditorContext::SetUserContext(bool globalSpace)
{
...
void nodeEditor::EditorContext::EnableShortcuts(bool enable)
{
    m_ShortcutsEnabled = enable;
}

bool nodeEditor::EditorContext::AreShortcutsEnabled()
{
    return m_ShortcutsEnabled;
}

nodeEditor::Control nodeEditor::EditorContext::BuildControl(bool allowOffscreen)
{
    m_IsHovered = false;
    m_IsHoveredWithoutOverlapp = false;
...
void nodeEditor::EditorContext::ShowMetrics(const Control& control)
{
    auto& io = ImGui::GetIO();
...
//
//------------------------------------------------------------------------------
void nodeEditor::NodeSettings::ClearDirty()
{
    m_IsDirty     = false;
    m_DirtyReason = SaveReasonFlags::None;
}

void nodeEditor::NodeSettings::MakeDirty(SaveReasonFlags reason)
{
    m_IsDirty     = true;
    m_DirtyReason = m_DirtyReason | reason;
}

nodeEditor::json::value nodeEditor::NodeSettings::Serialize()
{
    json::value result;
...
bool nodeEditor::NodeSettings::Parse(const std::string& string, NodeSettings& settings)
{
...
bool nodeEditor::NodeSettings::Parse(const json::value& data, NodeSettings& result)
{
    if (!data.is_object())
        return false;
...
//
//------------------------------------------------------------------------------
nodeEditor::NodeSettings* nodeEditor::Settings::AddNode(NodeId id)
{
    m_Nodes.push_back(NodeSettings(id));
    return &m_Nodes.back();
}

nodeEditor::NodeSettings* nodeEditor::Settings::FindNode(NodeId id)
{
...
void nodeEditor::Settings::RemoveNode(NodeId id)
{
    auto node = FindNode(id);
    if (!node)
        return;
...
void nodeEditor::Settings::ClearDirty(Node* node)
{
    if (node)
    {
...
void nodeEditor::Settings::MakeDirty(SaveReasonFlags reason, Node* node)
{
    m_IsDirty     = true;
    m_DirtyReason = m_DirtyReason | reason;
...
std::string nodeEditor::Settings::Serialize()
{
    json::value result;
...
bool nodeEditor::Settings::Parse(const std::string& string, Settings& settings)
{
...
//
//------------------------------------------------------------------------------
nodeEditor::Animation::Animation(EditorContext* editor):
    Editor(editor),
    m_State(Stopped),
    m_Time(0.0f),
    m_Duration(0.0f)
...
nodeEditor::Animation::~Animation()
{
    Stop();
}

void nodeEditor::Animation::Play(float duration)
{
    if (IsPlaying())
        Stop();
...
void nodeEditor::Animation::Stop()
{
    if (!IsPlaying())
        return;
...
void nodeEditor::Animation::Finish()
{
    if (!IsPlaying())
        return;
...
void nodeEditor::Animation::Update()
{
    if (!IsPlaying())
        return;
...
//
//------------------------------------------------------------------------------
nodeEditor::NavigateAnimation::NavigateAnimation(EditorContext* editor, NavigateAction& scrollAction):
    Animation(editor),
    Action(scrollAction)
{
}

void nodeEditor::NavigateAnimation::NavigateTo(const ImRect& target, float duration)
{
    Stop();
...
void nodeEditor::NavigateAnimation::OnUpdate(float progress)
{
    ImRect current;
...
void nodeEditor::NavigateAnimation::OnStop()
{
    Editor->MakeDirty(SaveReasonFlags::Navigation);
}

void nodeEditor::NavigateAnimation::OnFinish()
{
...
//
//------------------------------------------------------------------------------
nodeEditor::FlowAnimation::FlowAnimation(FlowAnimationController* controller):
...
void nodeEditor::FlowAnimation::Flow(ed::Link* link, float markerDistance, float speed, float duration)
{
    Stop();
...
void nodeEditor::FlowAnimation::Draw(ImDrawList* drawList)
{
...
bool nodeEditor::FlowAnimation::IsLinkValid() const
{
    return m_Link && m_Link->m_IsLive;
}

bool nodeEditor::FlowAnimation::IsPathValid() const
{
    return m_Path.size() > 1 && m_PathLength > 0.0f && m_Link->m_Start == m_LastStart && m_Link->m_End == m_LastEnd;
}

void nodeEditor::FlowAnimation::UpdatePath()
{
    if (!IsLinkValid())
    {
        ClearPath();
...
void nodeEditor::FlowAnimation::ClearPath()
{
    vector<CurvePoint>().swap(m_Path);
    m_PathLength = 0.0f;
}

ImVec2 nodeEditor::FlowAnimation::SamplePath(float distance) const
{
...
void nodeEditor::FlowAnimation::OnUpdate(float progress)
{
    IM_UNUSED(progress);
...
void nodeEditor::FlowAnimation::OnStop()
{
...
//
//------------------------------------------------------------------------------
nodeEditor::FlowAnimationController::FlowAnimationController(EditorContext* editor):
    AnimationController(editor)
{
}

nodeEditor::FlowAnimationController::~FlowAnimationController()
{
    for (auto animation : m_Animations)
        delete animation;
}

void nodeEditor::FlowAnimationController::Flow(Link* link, FlowDirection direction)
{
...
void nodeEditor::FlowAnimationController::Draw(ImDrawList* drawList)
{
...
nodeEditor::FlowAnimation* nodeEditor::FlowAnimationController::GetOrCreate(Link* link)
{
...
void nodeEditor::FlowAnimationController::Release(FlowAnimation* animation)
{
    IM_UNUSED(animation);
}
...
//
//------------------------------------------------------------------------------
const float nodeEditor::NavigateAction::s_DefaultZoomLevels[] =
{
    0.1f, 0.15f, 0.20f, 0.25f, 0.33f, 0.5f, 0.75f, 1.0f, 1.25f, 1.50f, 2.0f, 2.5f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f
};

const int nodeEditor::NavigateAction::s_DefaultZoomLevelCount = sizeof(s_DefaultZoomLevels) / sizeof(*s_DefaultZoomLevels);

nodeEditor::NavigateAction::NavigateAction(EditorContext* editor, ImGuiEx::Canvas& canvas):
    EditorAction(editor),
    m_IsActive(false),
    m_Zoom(1),
    m_VisibleRect(),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::NavigateAction::Accept(const Control& control)
{
...
bool nodeEditor::NavigateAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
bool nodeEditor::NavigateAction::HandleZoom(const Control& control)
{
    IM_UNUSED(control);
...
void nodeEditor::NavigateAction::ShowMetrics()
{
...
void nodeEditor::NavigateAction::NavigateTo(const ImRect& bounds, ZoomMode zoomMode, float duration, NavigationReason reason)
{
    if (ImRect_IsEmpty(bounds))
        return;
...
void nodeEditor::NavigateAction::NavigateTo(const ImRect& target, float duration, NavigationReason reason)
{
    m_Reason = reason;
...
void nodeEditor::NavigateAction::StopNavigation()
{
    m_Animation.Stop();
}

void nodeEditor::NavigateAction::FinishNavigation()
{
    m_Animation.Finish();
}

bool nodeEditor::NavigateAction::MoveOverEdge(const ImVec2& canvasSize)
{
...
void nodeEditor::NavigateAction::StopMoveOverEdge()
{
    if (m_MovingOverEdge)
    {
        Editor->MakeDirty(SaveReasonFlags::Navigation);
...
void nodeEditor::NavigateAction::SetWindow(ImVec2 position, ImVec2 size)
{
    m_WindowScreenPos  = position;
    m_WindowScreenSize = size;
}

ImGuiEx::CanvasView nodeEditor::NavigateAction::GetView() const
{
    return ImGuiEx::CanvasView(-m_Scroll, m_Zoom);
}

ImVec2 nodeEditor::NavigateAction::GetViewOrigin() const
{
    return -m_Scroll;
}

float nodeEditor::NavigateAction::GetViewScale() const
{
    return m_Zoom;
}

void nodeEditor::NavigateAction::SetViewRect(const ImRect& rect)
{
...
ImRect nodeEditor::NavigateAction::GetViewRect() const
{
    return m_Canvas.CalcViewRect(GetView());
}

float nodeEditor::NavigateAction::GetNextZoom(float steps)
{
...
float nodeEditor::NavigateAction::MatchSmoothZoom(float steps)
{
...
float nodeEditor::NavigateAction::MatchZoom(int steps, float fallbackZoom)
{
...
int nodeEditor::NavigateAction::MatchZoomIndex(int direction)
{
    int   bestIndex    = -1;
...
//
//------------------------------------------------------------------------------
nodeEditor::SizeAction::SizeAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_Clean(false),
    m_SizedNode(nullptr),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::SizeAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool nodeEditor::SizeAction::Process(const Control& control)
{
    if (m_Clean)
    {
        m_Clean = false;
...
void nodeEditor::SizeAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
nodeEditor::NodeRegion nodeEditor::SizeAction::GetRegion(Node* node)
{
    return node->GetRegion(ImGui::GetMousePos());
}

ImGuiMouseCursor nodeEditor::SizeAction::ChooseCursor(NodeRegion region)
{
    switch (region)
    {
        default:
...
//
//------------------------------------------------------------------------------
nodeEditor::DragAction::DragAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_Clear(false),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::DragAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool nodeEditor::DragAction::Process(const Control& control)
{
    if (m_Clear)
    {
        m_Clear = false;
...
void nodeEditor::DragAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
//
//------------------------------------------------------------------------------
nodeEditor::SelectAction::SelectAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::SelectAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool nodeEditor::SelectAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void nodeEditor::SelectAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
void nodeEditor::SelectAction::Draw(ImDrawList* drawList)
{
...
//
//------------------------------------------------------------------------------
nodeEditor::ContextMenuAction::ContextMenuAction(EditorContext* editor):
    EditorAction(editor),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::ContextMenuAction::Accept(const Control& control)
{
...
bool nodeEditor::ContextMenuAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void nodeEditor::ContextMenuAction::Reject()
{
    m_CandidateMenu = None;
    m_CurrentMenu   = None;
    m_ContextId     = ObjectId();
...
void nodeEditor::ContextMenuAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
bool nodeEditor::ContextMenuAction::ShowNodeContextMenu(NodeId* nodeId)
{
...
bool nodeEditor::ContextMenuAction::ShowPinContextMenu(PinId* pinId)
{
...
bool nodeEditor::ContextMenuAction::ShowLinkContextMenu(LinkId* linkId)
{
...
bool nodeEditor::ContextMenuAction::ShowBackgroundContextMenu()
{
...
//
//------------------------------------------------------------------------------
nodeEditor::ShortcutAction::ShortcutAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
    m_InAction(false),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::ShortcutAction::Accept(const Control& control)
{
...
bool nodeEditor::ShortcutAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void nodeEditor::ShortcutAction::Reject()
{
    m_IsActive        = false;
    m_CurrentAction   = None;
    m_Context.resize(0);
...
void nodeEditor::ShortcutAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
bool nodeEditor::ShortcutAction::Begin()
{
    if (m_IsActive)
        m_InAction = true;
    return m_IsActive;
...
void nodeEditor::ShortcutAction::End()
{
    if (m_IsActive)
        m_InAction = false;
}

bool nodeEditor::ShortcutAction::AcceptCut()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Cut;
}

bool nodeEditor::ShortcutAction::AcceptCopy()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Copy;
}

bool nodeEditor::ShortcutAction::AcceptPaste()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Paste;
}

bool nodeEditor::ShortcutAction::AcceptDuplicate()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Duplicate;
}

bool nodeEditor::ShortcutAction::AcceptCreateNode()
{
    IM_ASSERT(m_InAction);
...
//
//------------------------------------------------------------------------------
nodeEditor::CreateItemAction::CreateItemAction(EditorContext* editor):
    EditorAction(editor),
    m_InActive(false),
    m_NextStage(None),
    m_CurrentStage(None),
...
nodeEditor::EditorAction::AcceptResult nodeEditor::CreateItemAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool nodeEditor::CreateItemAction::Process(const Control& control)
{
    IM_ASSERT(m_IsActive);
...
void nodeEditor::CreateItemAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
void nodeEditor::CreateItemAction::SetStyle(ImU32 color, float thickness)
{
    m_LinkColor     = color;
    m_LinkThickness = thickness;
}

bool nodeEditor::CreateItemAction::Begin()
{
...
void nodeEditor::CreateItemAction::End()
{
...
void nodeEditor::CreateItemAction::DragStart(Pin* startPin)
{
    IM_ASSERT(!m_InActive);
...
void nodeEditor::CreateItemAction::DragEnd()
{
    IM_ASSERT(!m_InActive);
...
void nodeEditor::CreateItemAction::DropPin(Pin* endPin)
{
    IM_ASSERT(!m_InActive);
...
void nodeEditor::CreateItemAction::DropNode()
{
    IM_ASSERT(!m_InActive);
...
void nodeEditor::CreateItemAction::DropNothing()
{
    IM_ASSERT(!m_InActive);
...
nodeEditor::CreateItemAction::Result nodeEditor::CreateItemAction::RejectItem()
{
    IM_ASSERT(m_InActive);
...
nodeEditor::CreateItemAction::Result nodeEditor::CreateItemAction::AcceptItem()
{
    IM_ASSERT(m_InActive);
...
nodeEditor::CreateItemAction::Result nodeEditor::CreateItemAction::QueryLink(PinId* startId, PinId* endId)
{
    IM_ASSERT(m_InActive);
...
nodeEditor::CreateItemAction::Result nodeEditor::CreateItemAction::QueryNode(PinId* pinId)
{
    IM_ASSERT(m_InActive);
...
//
//------------------------------------------------------------------------------
nodeEditor::DeleteItemsAction::DeleteItemsAction(EditorContext* editor):
    EditorAction(editor),
    m_IsActive(false),
...
void nodeEditor::DeleteItemsAction::DeleteDeadLinks(NodeId nodeId)
{
...
void nodeEditor::DeleteItemsAction::DeleteDeadPins(NodeId nodeId)
{
...
nodeEditor::EditorAction::AcceptResult nodeEditor::DeleteItemsAction::Accept(const Control& control)
{
    IM_ASSERT(!m_IsActive);
...
bool nodeEditor::DeleteItemsAction::Process(const Control& control)
{
    IM_UNUSED(control);
...
void nodeEditor::DeleteItemsAction::ShowMetrics()
{
    EditorAction::ShowMetrics();
...
bool nodeEditor::DeleteItemsAction::Add(Object* object)
{
...
bool nodeEditor::DeleteItemsAction::Begin()
{
    if (!m_IsActive)
        return false;
...
void nodeEditor::DeleteItemsAction::End()
{
    if (!m_IsActive)
        return;
...
bool nodeEditor::DeleteItemsAction::QueryLink(LinkId* linkId, PinId* startId, PinId* endId)
{
    ObjectId objectId;
...
bool nodeEditor::DeleteItemsAction::QueryNode(NodeId* nodeId)
{
    ObjectId objectId;
...
bool nodeEditor::DeleteItemsAction::QueryItem(ObjectId* itemId, IteratorType itemType)
{
    if (!m_InInteraction)
        return false;
...
bool nodeEditor::DeleteItemsAction::AcceptItem(bool deleteDependencies)
{
    if (!m_InInteraction)
        return false;
...
void nodeEditor::DeleteItemsAction::RejectItem()
{
    if (!m_InInteraction)
        return;
...
void nodeEditor::DeleteItemsAction::RemoveItem(bool deleteDependencies)
{
...
nodeEditor::Object* nodeEditor::DeleteItemsAction::DropCurrentItem()
{
    auto item = m_CandidateObjects[m_CandidateItemIndex];
...
//
//------------------------------------------------------------------------------
nodeEditor::NodeBuilder::NodeBuilder(EditorContext* editor):
    Editor(editor),
...
nodeEditor::NodeBuilder::~NodeBuilder()
{
    m_Splitter.ClearFreeMemory();
    m_PinSplitter.ClearFreeMemory();
}

void nodeEditor::NodeBuilder::Begin(NodeId nodeId)
{
...
void nodeEditor::NodeBuilder::End()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
void nodeEditor::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
void nodeEditor::NodeBuilder::EndPin()
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::PinRect(const ImVec2& a, const ImVec2& b)
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::PinPivotRect(const ImVec2& a, const ImVec2& b)
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::PinPivotSize(const ImVec2& size)
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::PinPivotScale(const ImVec2& scale)
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::PinPivotAlignment(const ImVec2& alignment)
{
    IM_ASSERT(nullptr != m_CurrentPin);
...
void nodeEditor::NodeBuilder::Group(const ImVec2& size)
{
    IM_ASSERT(nullptr != m_CurrentNode);
    IM_ASSERT(nullptr == m_CurrentPin);
    IM_ASSERT(false   == m_IsGroup);
...
ImDrawList* nodeEditor::NodeBuilder::GetUserBackgroundDrawList() const
{
    return GetUserBackgroundDrawList(m_CurrentNode);
}

ImDrawList* nodeEditor::NodeBuilder::GetUserBackgroundDrawList(Node* node) const
{
...
//
//------------------------------------------------------------------------------
nodeEditor::HintBuilder::HintBuilder(EditorContext* editor):
    Editor(editor),
    m_IsActive(false),
    m_CurrentNode(nullptr)
{
...
bool nodeEditor::HintBuilder::Begin(NodeId nodeId)
{
    IM_ASSERT(nullptr == m_CurrentNode);
...
void nodeEditor::HintBuilder::End()
{
    if (!m_IsActive)
        return;
...
ImVec2 nodeEditor::HintBuilder::GetGroupMin()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImVec2 nodeEditor::HintBuilder::GetGroupMax()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImDrawList* nodeEditor::HintBuilder::GetForegroundDrawList()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
ImDrawList* nodeEditor::HintBuilder::GetBackgroundDrawList()
{
    IM_ASSERT(nullptr != m_CurrentNode);
...
//
//------------------------------------------------------------------------------
void nodeEditor::Style::PushColor(StyleColor colorIndex, const ImVec4& color)
{
...
void nodeEditor::Style::PopColor(int count)
{
    while (count > 0)
    {
...
void nodeEditor::Style::PushVar(StyleVar varIndex, float value)
{
...
void nodeEditor::Style::PushVar(StyleVar varIndex, const ImVec2& value)
{
...
void nodeEditor::Style::PushVar(StyleVar varIndex, const ImVec4& value)
{
...
void nodeEditor::Style::PopVar(int count)
{
    while (count > 0)
    {
...
const char* nodeEditor::Style::GetColorName(StyleColor colorIndex) const
{
    switch (colorIndex)
    {
...
float* nodeEditor::Style::GetVarFloatAddr(StyleVar idx)
{
    switch (idx)
    {
...
ImVec2* nodeEditor::Style::GetVarVec2Addr(StyleVar idx)
{
    switch (idx)
    {
...
ImVec4* nodeEditor::Style::GetVarVec4Addr(StyleVar idx)
{
    switch (idx)
    {
...
//
//------------------------------------------------------------------------------
nodeEditor::Config::Config(const ax::NodeEditor::Config* config)
{
    if (config)
        *static_cast<ax::NodeEditor::Config*>(this) = *config;
}

std::string nodeEditor::Config::Load()
{
    std::string data;
...
std::string nodeEditor::Config::LoadNode(NodeId nodeId)
{
    std::string data;
...
void nodeEditor::Config::BeginSave()
{
    if (BeginSaveSession)
        BeginSaveSession(UserPointer);
}

bool nodeEditor::Config::Save(const std::string& data, SaveReasonFlags flags)
{
    if (SaveSettings)
    {
...
bool nodeEditor::Config::SaveNode(NodeId nodeId, const std::string& data, SaveReasonFlags flags)
{
    if (SaveNodeSettings)
...
void nodeEditor::Config::EndSave()
{
    if (EndSaveSession)
